...
await page.goto('https://www.ontheplayground.studio/', { waitUntil: 'networkidle', timeout: 45000 }).catch(() => {})
...
await page.evaluate(async (labels) => {
  ...
  for (const v of document.querySelectorAll('video')) {
    try { v.muted = true; v.playsInline = true; await v.play().catch(() => {}) } catch {}
  }
}, CONSENT)
await page.waitForTimeout(5000)
const restricted = await page.evaluate(() => document.body.innerText.includes("couldn't verify the security"))
