...
/** Dark closer — bridges into the site footer. */
export function StudioCta({subhead, buttonLabel, link}: StudioCtaProps) {
  if (!subhead && !buttonLabel) return null

  const label = buttonLabel?.replace(/\s*→\s*$/, '') || null

  return (
    <section className="border-t border-black/10 bg-[#6B6B6B]">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-6 px-5 py-12 text-center sm:gap-7 sm:px-6 sm:py-14 md:flex-row md:items-end md:justify-between md:gap-10 md:px-10 md:py-14 md:text-left">
        ...
        <CapReveal delay={0.08} y={8} className="shrink-0">
          {label ? (
            <Link
              href={link || '/contact'}
              className="group inline-flex min-h-11 items-center justify-center gap-2 border border-white/50 px-6 py-3 font-mono text-[11px] uppercase tracking-[0.18em] text-white transition-colors duration-300 hover:border-white hover:bg-white hover:text-black"
            >
              <span>{label}</span>
              <span
                aria-hidden="true"
