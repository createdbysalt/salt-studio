...
const DEFAULT_SUCCESS_HEADLINE = "You're on the list."
      ...
      >
        {visibleFields.map((field) => (
          <FormFieldInput
            ...
            page={isPage}
            wide={!isPage && (field.type === 'textarea' || field.type === 'multiselect')}
          />
        ))}
      </div>
      ...
      {field.type === 'textarea' ? (
        ...
        <fieldset className="mt-1 space-y-2">
          ...
          {(field.options ?? []).map((option) => (
            <label
              key={option}
              className="flex cursor-pointer items-start gap-3 text-base text-foreground sm:text-[0.9375rem]"
            >
              <input
                type="checkbox"
                name={field.name}
                value={option}
                className="mt-1 size-4 shrink-0 border-foreground/30 text-foreground accent-foreground"
              />
              <span>{option}</span>
            </label>
          ))}
